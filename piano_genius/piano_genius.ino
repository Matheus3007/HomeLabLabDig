...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic,(user+"/S0").c_str())==0) {
    if ((char)payload[0] == '1') {
      tone(D7, NOTE_FS6, 250); //125 ou 250
    }
  }

  if (strcmp(topic,(user+"/S1").c_str())==0) {
    if ((char)payload[0] == '1') {
      tone(D7, NOTE_C4, 250); //125 ou 250
    }
  }

  if (strcmp(topic,(user+"/S2").c_str())==0) {
    if ((char)payload[0] == '1') {
      tone(D7, NOTE_DS5, 250); //125 ou 250
    }
  }

  if (strcmp(topic,(user+"/S3").c_str())==0) {
    if ((char)payload[0] == '1') {
      tone(D7, NOTE_A7, 250); //125 ou 250
    }
  }
  
  // Controle leds
  
  if (strcmp(topic,(user+"/S4").c_str())==0) {
    if ((char)payload[0] == '1') {
  ...
  if (strcmp(topic,(user+"/S5").c_str())==0) {
    if ((char)payload[0] == '1') {
  ...
  if (strcmp(topic,(user+"/S6").c_str())==0) {
    if ((char)payload[0] == '1') {
  ...
  if (strcmp(topic,(user+"/S7").c_str())==0) {
    if ((char)payload[0] == '1') {
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), user.c_str(), passwd.c_str())) {
      ...
      client.subscribe((user+"/S4").c_str());
      client.subscribe((user+"/S5").c_str());
      client.subscribe((user+"/S6").c_str());
      client.subscribe((user+"/S7").c_str());
...
void setup() {
  ...
  pinMode(D3, OUTPUT); // Led 0 topico S4
  pinMode(D4, OUTPUT); // Led 1 topico S5
  pinMode(D8, OUTPUT); // Led 2 topico S6
  pinMode(D0, OUTPUT); // Led 3 topico S7
